...
import { Account, Address, BIGINT_0, BIGINT_1, BIGINT_8, GWEI_TO_WEI, KECCAK256_RLP, bigIntToAddressBytes, bigIntToBytes, bytesToHex, concatBytes, equalsBytes, getVerkleTreeIndexesForStorageSlot, hexToBytes, intToBytes, setLengthLeft, short, unprefixedHexToBytes, } from '@ethereumjs/util';
...
export async function runBlock(opts) {
    ...
    if (this.common.hardforkIsActiveOnBlock(Hardfork.Dao, block.header.number) &&
    ...
    if (generateFields) {
        ...
        if (this.common.isActivatedEIP(7685)) {
            ...
            if (!valid) {
                ...
                if (this.DEBUG)
                    debug(`Invalid requestsRoot received=${bytesToHex(block.header.requestsRoot)} expected=${bytesToHex(validRoot)}`);
...
async function applyBlock(block, opts) {
    ...
    if (opts.reportPreimages === true) {
        ...
        blockResults.preimages.set(bytesToHex(this.evm.stateManager.getAppliedKey(block.header.coinbase.toBytes())), block.header.coinbase.toBytes());
 ...
 */
export async function accumulateParentBlockHash(currentBlockNumber, parentHash) {
    if (!this.common.isActivatedEIP(2935)) {
        throw new Error('Cannot call `accumulateParentBlockHash`: EIP 2935 is not active');
    }
    const historyAddress = new Address(bigIntToAddressBytes(this.common.param('vm', 'historyStorageAddress')));
