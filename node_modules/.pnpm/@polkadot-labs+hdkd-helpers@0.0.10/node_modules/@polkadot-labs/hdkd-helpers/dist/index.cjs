...
__export(src_exports, {
  BIP39_EN_WORDLIST: () => BIP39_EN_WORDLIST,
  DEV_MINI_SECRET: () => DEV_MINI_SECRET,
  DEV_PHRASE: () => DEV_PHRASE,
  accountId: () => accountId,
  blake2b256: () => blake2b256,
  blake2b512: () => blake2b512,
  createDerive: () => createDerive,
  createDeriveKeyPairFn: () => createDeriveKeyPairFn,
  ecdsa: () => ecdsa,
  ecdsaDerive: () => ecdsaDerive,
  ed25519: () => ed25519,
  ed25519Derive: () => ed25519Derive,
  ensureBytes: () => ensureBytes,
  entropyToMiniSecret: () => entropyToMiniSecret,
  entropyToMiniSecretAsync: () => entropyToMiniSecretAsync,
  entropyToMnemonic: () => entropyToMnemonic,
  generateMnemonic: () => generateMnemonic,
  mnemonicToEntropy: () => mnemonicToEntropy,
  mnemonicToMiniSecret: () => mnemonicToMiniSecret,
  mnemonicToMiniSecretAsync: () => mnemonicToMiniSecretAsync,
  parseSuri: () => parseSuri,
  sr25519: () => sr25519,
...
var ensureBytes = (title, hex, expectedLength) => {
  ...
  return (0, import_utils.ensureBytes)(title, hex, expectedLength);
};
...
var ss58Encode = (payload, prefix = 42) => {
  ...
  return import_base.base58.encode(Uint8Array.of(...prefixBytes, ...payload, ...checksum));
};
var VALID_ADDRESS_LENGTHS = [35, 36, 37];
var ss58Decode = (addressStr) => {
  const address = import_base.base58.decode(addressStr);
...
var deriveChecksumBits = (entropy) => (0, import_sha256.sha256)(entropy)[0].toString(2).padStart(8, "0").slice(0, entropy.length / 4);
...
var generateMnemonic = (strength = 128, wordlist = BIP39_EN_WORDLIST) => entropyToMnemonic((0, import_utils3.randomBytes)(strength / 8), wordlist);
...
var entropyToMiniSecret = (entropy, password = "") => (0, import_pbkdf2.pbkdf2)(import_sha512.sha512, entropy, salt(password), {
  c: 2048,
  dkLen: 64
}).slice(0, 32);
var entropyToMiniSecretAsync = async (entropy, password = "") => (await (0, import_pbkdf2.pbkdf2Async)(import_sha512.sha512, entropy, salt(password), {
  c: 2048,
  dkLen: 64
})).slice(0, 32);
...
var ecdsa = {
  getPublicKey(privateKey) {
    return import_secp256k1.secp256k1.getPublicKey(ensureBytes("privateKey", privateKey));
  },
  sign(message, privateKey) {
    const signature = import_secp256k1.secp256k1.sign(
  ...
  verify(signature, message, publicKey) {
    return import_secp256k1.secp256k1.verify(
      import_secp256k1.secp256k1.Signature.fromCompact(
...
var ed25519 = {
  getPublicKey(privateKey) {
    return import_ed25519.ed25519.getPublicKey(ensureBytes("privateKey", privateKey));
  },
  sign(message, privateKey) {
    return import_ed25519.ed25519.sign(
...
var sr25519 = {
  getPublicKey(privateKey) {
    return (0, import_micro_sr25519.getPublicKey)(ensureBytes("privateKey", privateKey, 64));
  },
  sign(message, privateKey) {
    return (0, import_micro_sr25519.sign)(
      ensureBytes("privateKey", privateKey, 64),
      ensureBytes("message", message)
    );
  },
  verify(signature, message, publicKey) {
    return (0, import_micro_sr25519.verify)(
      ensureBytes("message", message),
...
var import_micro_sr255192 = require("micro-sr25519");
var sr25519Derive = (seed, curve, derivations) => {
  const privateKey = derivations.reduce((secretKey, [type, chainCode]) => {
    const deriveFn = type === "hard" ? import_micro_sr255192.HDKD.secretHard : import_micro_sr255192.HDKD.secretSoft;
    return deriveFn(secretKey, chainCode);
  }, (0, import_micro_sr255192.secretFromSeed)(seed));
  const publicKey = (0, import_micro_sr255192.getPublicKey)(privateKey);
  return {
    publicKey,
    sign(message) {
      return curve.sign(message, privateKey);
...
var createChainCode = (code) => {
  ...
  chainCode.set(Number.isNaN(+code) ? import_scale_ts2.str.enc(code) : import_scale_ts2.u32.enc(+code));
  return chainCode;
};
