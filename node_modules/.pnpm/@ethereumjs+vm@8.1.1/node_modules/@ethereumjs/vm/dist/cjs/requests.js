...
const accumulateRequests = async (vm, txResults) => {
    ...
    if (common.isActivatedEIP(6110)) {
        const depositContractAddress = vm.common['_chainParams'].depositContractAddress ??
            common_1.Common.getInitializedChains().mainnet.depositContractAddress;
...
const accumulateEIP7002Requests = async (vm, requests) => {
    ...
    const addressBytes = (0, util_1.setLengthLeft)((0, util_1.bigIntToBytes)(vm.common.param('vm', 'withdrawalRequestPredeployAddress')), 20);
    const withdrawalsAddress = util_1.Address.fromString((0, util_1.bytesToHex)(addressBytes));
    ...
    const systemAddressBytes = (0, util_1.bigIntToAddressBytes)(vm.common.param('vm', 'systemAddress'));
    const systemAddress = util_1.Address.fromString((0, util_1.bytesToHex)(systemAddressBytes));
    ...
    if (resultsBytes.length > 0) {
        ...
        for (let startByte = 0; startByte < resultsBytes.length; startByte += 76) {
            ...
            const amount = (0, util_1.bytesToBigInt)((0, util_1.unpadBytes)(slicedBytes.slice(68, 76))); // 8 Bytes / Uint64
            requests.push(util_1.WithdrawalRequest.fromRequestData({ sourceAddress, validatorPubkey, amount }));
        }
    }
    if (addrIsEmpty) {
...
const accumulateEIP7251Requests = async (vm, requests) => {
    // Partial withdrawals logic
    const addressBytes = (0, util_1.setLengthLeft)((0, util_1.bigIntToBytes)(vm.common.param('vm', 'consolidationRequestPredeployAddress')), 20);
    const consolidationsAddress = util_1.Address.fromString((0, util_1.bytesToHex)(addressBytes));
    ...
    if (resultsBytes.length > 0) {
        ...
        for (let startByte = 0; startByte < resultsBytes.length; startByte += 116) {
            ...
            requests.push(util_1.ConsolidationRequest.fromRequestData({ sourceAddress, sourcePubkey, targetPubkey }));
        }
    }
    if (addrIsEmpty) {
        await vm.stateManager.deleteAccount(systemAddress);
...
const accumulateDeposits = async (depositContractAddress, txResults, requests) => {
    for (const [_, tx] of txResults.entries()) {
        for (let i = 0; i < tx.receipt.logs.length; i++) {
            ...
            if ((0, util_1.bytesToHex)(log[0]).toLowerCase() === depositContractAddress.toLowerCase()) {
                ...
                const pubKeyIdx = (0, util_1.bytesToInt)(log[2].slice(0, 32));
                const pubKeySize = (0, util_1.bytesToInt)(log[2].slice(pubKeyIdx, pubKeyIdx + 32));
                const withdrawalCredsIdx = (0, util_1.bytesToInt)(log[2].slice(32, 64));
                const withdrawalCredsSize = (0, util_1.bytesToInt)(log[2].slice(withdrawalCredsIdx, withdrawalCredsIdx + 32));
                const amountIdx = (0, util_1.bytesToInt)(log[2].slice(64, 96));
                const amountSize = (0, util_1.bytesToInt)(log[2].slice(amountIdx, amountIdx + 32));
                const sigIdx = (0, util_1.bytesToInt)(log[2].slice(96, 128));
                const sigSize = (0, util_1.bytesToInt)(log[2].slice(sigIdx, sigIdx + 32));
                const indexIdx = (0, util_1.bytesToInt)(log[2].slice(128, 160));
                const indexSize = (0, util_1.bytesToInt)(log[2].slice(indexIdx, indexIdx + 32));
                ...
                const amount = (0, util_1.bytesToBigInt)(amountBytesBigEndian);
                ...
                const index = (0, util_1.bytesToBigInt)(indexBytesBigEndian);
                requests.push(util_1.DepositRequest.fromRequestData({
                    pubkey,
