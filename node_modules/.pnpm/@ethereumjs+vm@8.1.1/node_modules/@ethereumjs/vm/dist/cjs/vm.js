...
class VM {
    ...
    static async create(opts = {}) {
        ...
        if (opts.common === undefined) {
            opts.common = new common_1.Common({ chain: common_1.Chain.Mainnet });
        }
        if (opts.stateManager === undefined) {
            opts.stateManager = new statemanager_1.DefaultStateManager({ common: opts.common });
        }
        if (opts.blockchain === undefined) {
            opts.blockchain = await blockchain_1.Blockchain.create({ common: opts.common });
        }
        ...
        if (opts.evm === undefined) {
            ...
            if (opts.profilerOpts?.reportAfterBlock === true ||
                opts.profilerOpts?.reportAfterTx === true) {
                enableProfiler = true;
            }
            const evmOpts = opts.evmOpts ?? {};
            opts.evm = await evm_1.EVM.create({
                common: opts.common,
    ...
    async shallowCopy(downlevelCaches = true) {
        ...
        const evmCopy = await evm_1.EVM.create(evmOpts); // TODO fixme (should copy the EVMInterface, not default EVM)
        return VM.create({
            stateManager,
