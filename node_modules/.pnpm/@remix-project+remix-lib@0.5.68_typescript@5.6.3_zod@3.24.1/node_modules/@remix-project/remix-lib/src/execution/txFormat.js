...
exports.REGEX_DECIMAL = exports.REGEX_SCIENTIFIC = exports.normalizeParam = void 0;
...
const from_exponential_1 = tslib_1.__importDefault(require("from-exponential"));
/**
    ...
    if (fnabi.outputs && fnabi.outputs.length > 0) {
        try {
            ...
            if (!response || !response.length)
                response = new Uint8Array(32 * fnabi.outputs.length); // ensuring the data is at least filled by 0 cause `AbiCoder` throws if there's not enough data
            // decode data
    ...
    for (let i = 0; i < params.length; i++) {
        ...
        if (params.charAt(i) === '"') {
            ...
            for (let j = i + 1; !endQuoteIndex; j++) {
                if (params.charAt(j) === '"') {
                    args.push((0, exports.normalizeParam)(params.substring(i + 1, j)));
                    endQuoteIndex = true;
                    i = j;
                }
            ...
            if (startIndex >= 0) {
                ...
                param = (0, exports.normalizeParam)(param);
                args.push(param);
            }
...
const normalizeParam = (param) => {
    ...
    // fromExponential
    if (!param.startsWith('0x')) {
        const regSci = exports.REGEX_SCIENTIFIC.exec(param);
        const exponents = regSci ? regSci[2] : null;
        if (regSci && exports.REGEX_DECIMAL.exec(exponents)) {
            try {
                let paramTrimmed = param.replace(/^'/g, '').replace(/'$/g, '');
                paramTrimmed = paramTrimmed.replace(/^"/g, '').replace(/"$/g, '');
                param = (0, from_exponential_1.default)(paramTrimmed);
            }
            catch (e) {
                console.log(e);
            }
...
exports.REGEX_SCIENTIFIC = /^-?(\d+\.?\d*)e\d*(\d+)$/;
exports.REGEX_DECIMAL = /^\d*/;
