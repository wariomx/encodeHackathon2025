...
exports.encodeReceipt = exports.rewardAccount = exports.calculateMinerReward = exports.accumulateParentBeaconBlockRoot = exports.accumulateParentBlockHash = exports.runBlock = void 0;
...
async function runBlock(opts) {
    ...
    if (this.common.isActivatedEIP(6800)) {
        if (!(stateManager instanceof statemanager_1.StatelessVerkleStateManager)) {
    ...
    if (this.common.hardforkIsActiveOnBlock(common_1.Hardfork.Dao, block.header.number) &&
    ...
    if (block.common.isActivatedEIP(7685)) {
        requests = await (0, requests_js_1.accumulateRequests)(this, result.results);
        requestsRoot = await block_1.Block.genRequestsTrieRoot(requests);
    }
    // Persist state
    await this.evm.journal.commit();
    if (this.DEBUG) {
    ...
    if (generateFields) {
        ...
        if (this.common.isActivatedEIP(7685)) {
            ...
            if (!valid) {
                const validRoot = await block_1.Block.genRequestsTrieRoot(requests);
                if (this.DEBUG)
                    debug(`Invalid requestsRoot received=${(0, util_1.bytesToHex)(block.header.requestsRoot)} expected=${(0, util_1.bytesToHex)(validRoot)}`);
...
async function applyBlock(block, opts) {
    ...
    if (opts.reportPreimages === true) {
        ...
        blockResults.preimages.set((0, util_1.bytesToHex)(this.evm.stateManager.getAppliedKey(block.header.coinbase.toBytes())), block.header.coinbase.toBytes());
 ...
 */
async function accumulateParentBlockHash(currentBlockNumber, parentHash) {
    if (!this.common.isActivatedEIP(2935)) {
        throw new Error('Cannot call `accumulateParentBlockHash`: EIP 2935 is not active');
    }
    const historyAddress = new util_1.Address((0, util_1.bigIntToAddressBytes)(this.common.param('vm', 'historyStorageAddress')));
    ...
    try {
        if ((await this.stateManager.getAccount(historyAddress)) === undefined) {
            const emptyHistoryAcc = new util_1.Account(BigInt(1));
    ...
    async function putBlockHash(vm, hash, number) {
        ...
        if (vm.common.isActivatedEIP(6800)) {
            const { treeIndex, subIndex } = (0, util_1.getVerkleTreeIndexesForStorageSlot)(ringKey);
            vm.stateManager.accessWitness.touchAddressOnWriteAndComputeGas(historyAddress, treeIndex, subIndex);
        }
    ...
    await putBlockHash(this, parentHash, currentBlockNumber - util_1.BIGINT_1);
